...
int main(int argc , char *argv[]) {
	...
	SOCKET sockfd;
	int msg_len, iResult, nbytes, result, addr_len, i, ip_idx, port_idx, msg_idx, SERV_UDP_PORT, rec_serv_len, rcvbf;
	...
	nbytes = recvfrom(sockfd, (char*)&rcvbf, sizeof(buffer), 0, (struct sockaddr*)&rec_serv_addr, &rec_serv_len);
	...
	if (strcmp(inet_ntoa(rec_serv_addr.sin_addr), SERV_HOST_ADDR) == 0  && ntohs(rec_serv_addr.sin_port) == SERV_UDP_PORT)
		printf("<CLI> Mensagem {%d} recebida do servidor!\n", rcvbf);
	else
		printf("<CLI> Mensagem {%d} recebida do impostor com IP: {%s} e Port: {%s}.\n", rcvbf, inet_ntoa(rec_serv_addr.sin_addr), ntohs(rec_serv_addr.sin_port));
